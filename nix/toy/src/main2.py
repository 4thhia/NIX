...
@hydra.main(config_path="../configs/_logs/multi_aux/.hydra", config_name="config", version_base=None)
def main(cfg: DictConfig) -> None:
    ...
    def update_step(params, weights, lmb, activation):
        ...
        def loss_fn(weights):
            if activation == "sigmoid":
                weighted_aux_grad = jnp.sum(jnp.array([nn.sigmoid(w) * g for w, g in zip(weights, aux_grads)]), axis=0)
            elif activation == "tanh":
                weighted_aux_grad = jnp.sum(jnp.array([nn.tanh(w) * g for w, g in zip(weights, aux_grads)]), axis=0)
            else:
        ...
        return next_params, next_weights, next_lmb, main_grad, weighted_aux_grad, main_loss
    ...
    for i in trange(cfg.training.max_iter):
        params, weights, lmb, main_grad, weighted_aux_grad, main_loss = update_step(params, weights, lmb, cfg.training.activation)

        for key, value in zip(["params", "weights", "lmbs", "main_grads", "weighted_aux_grads", "main_losses"], [np.array(params), np.array(weights), np.array(lmb), np.array(-main_grad), np.array(-weighted_aux_grad), np.array(main_loss)]):
            training_history[key].append(value)

    _, _, _, main_grad, weighted_aux_grad, main_loss = update_step(params, weights, lmb, cfg.training.activation)
    for key, value in zip(["main_grads", "weighted_aux_grads", "main_losses"], [np.array(-main_grad), np.array(-weighted_aux_grad)]):
            training_history[key].append(value)
    ...
    summary = {
        ...
        "main_loss": [main_loss.tolist() for main_loss in training_history["main_losses"]],
    }
