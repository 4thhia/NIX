...
from gits.toy.src.losses import create_loss_grad_fn, loss_fn_surface
...
def plot_lambda(cfg, history):
    ...
    plt.ylabel(r'$\lambda$')
    plt.legend()
    plt.savefig(f"out/{cfg.experiment_name}/{cfg.sub_experiment_name}/{cfg.experiment_name}.png")
...
def plot2D(cfg, history):
    ...
    # Save gif
    imageio.mimsave(f"out/{cfg.experiment_name}/{cfg.sub_experiment_name}/{cfg.experiment_name}.gif", gif_images, duration=0.1)
    plt.close(fig)
