...
def main(cfg: DictConfig) -> None:
    ...
    def train_step(x, weight, lmb, gamma_coef):
        ...
        gamma_coef += cfg.algorithm.gamma.coef.lr * (cfg.algorithm.target_loss - main_loss)
        gamma_coef = jnp.clip(gamma_coef, - cfg.algorithm.gamma.coef.bound, cfg.algorithm.gamma.coef.bound)
        gamma = (cfg.algorithm.target_loss > main_loss) * nn.sigmoid(gamma_coef) * cfg.algorithm.gamma.max

        lmb += jnp.maximum(0, cfg.optimizers.lr_lmb * (jnp.sum(main_grad_normalized * (cfg.algorithm.beta*main_grad - aux_grad)) - gamma))
        lmb = jnp.minimum(1, lmb)
        return x, activation_fn(weight), lmb, gamma_coef, weight_loss_fn
    ...
    for step in trange(cfg.training.max_iter):
        x, weight, lmb, gamma_coef, weight_loss_fn = train_step(x, weight, lmb, gamma_coef)
        ...
        ax[0].axhline(cfg.algorithm.target_loss, color='k', linestyle='--', label="Target Loss")
