...
def train(
    ...
    def train_step(runner_state, batch):
        ...
        def kld_loss_fn(mu, lnv):
            kld_loss = kld_coef * jnp.mean(jnp.sum(-0.5 * (1 + lnv - mu ** 2 - jnp.exp(lnv)), axis=1))
            return kld_loss
        ...
        def weight_loss_fn(ws):
            ...
            weight_loss = - jnp.mean(jnp.sum(main_grads * aux_grads, axis=-1))
        ...
        # Update network
        networks = networks.replace(encoder=new_encoder, decoder=new_decoder, classifier=new_classifier, weightunet=new_weightunet)
...
def evaluate(
    ...
    def evaluate_step(runner_state, batch):
        ...
        # Computes the KL divergence between the latent distribution and a standard normal distribution
        def kld_loss_fn():
            kld_loss = kld_coef * jnp.mean(jnp.sum(-0.5 * (1 + logvar - mean ** 2 - jnp.exp(logvar)), axis=1))
            return kld_loss
