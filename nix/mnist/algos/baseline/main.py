...
from nix.mnist.common.plotter import save_recons
from nix.mnist.common.utils import create_ckpt_mngr
from nix.mnist.datasets import create_loaders
from nix.mnist.algos.baseline.train import Networks, train, evaluate
...
def setup_networks(
    ...
    rng_encoder, rng_decoder, rng_classifier = jax.random.split(rng, num=3)
    ...
    return Networks(state_encoder, state_decoder, state_classifier)
...
def main(cfg: DictConfig) -> None:
    ...
    cfg.run_name = (
        ...
        f"__kld_coef={cfg.kld_coef}__lr_weightunet={cfg.optimizers.weightunet.lr}"
    ...
    for epoch_id in trange(cfg.training.num_epochs, leave=False):
        ...
        if (epoch_id == 0) or ((epoch_id + 1) % cfg.training.eval_interval == 0):
            ...
            save_recons(cfg, f"epoch{epoch_id}", networks, ordered_loader, best_acc)
    ...
    save_recons(cfg, "final", networks, ordered_loader, best_acc)
